...
int print_parameters(struct parameters* para, FILE* f)
{
    ...
    fprintf(f, "foc_plane q:   %8.3f m\n",  1./(1./para->crl.f-1./para->source.distance));
    fprintf(f, "\n");
...
//    fprintf(f, "pixel size: to be set by field prop\n");
    ...
    fprintf(f, "delta_f=%1.2fHz       \n",  1./para->crl.aperture);
    fprintf(f, "delta2=%1.6fμm       \n",  1./para->crl.aperture*para->xray.wavelength*para->detector.distance*1.e6);
    fprintf(f, "\n");
...
int show_field(struct field* argfield, int start, int nval, char* message){
int ret=0;
int i;
printf("   ******************************\n");
printf("   Field in %s \n", message);
...
printf("      show %d {Re} {Im} values [%d,%d]: \n", nval, start, start+nval-1);
for(i=start;i<(start+nval);i++){
